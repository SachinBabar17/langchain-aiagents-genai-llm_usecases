...
analysis_prompt = PromptTemplate(
    template="""You are an expert code reviewer. Analyze the following {language} code and identify potential issues.
...
- Coding best practices specific to {language}
...
def main():
    ...
    if analyze_button and code_input.strip():
        with results_container:
            try:
                ...
                with st.spinner("Analyzing code..."):
                    analysis_result = analysis_chain.invoke({"code": code_input, "language": language})
                ...
                if show_stages:
                    with st.expander("Detailed Analysis", expanded=False):
                        ...
                        for idx, issue in enumerate(analysis_result.issues, 1):
                            st.markdown(f"""
                            **{idx}. Line {issue.line_number}** - *{issue.issue_type}* ({issue.severity})
        ...
        results_container.warning("Please enter some code to review!")
